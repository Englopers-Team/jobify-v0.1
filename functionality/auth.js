...
obj.personSignUp = (req, res) => {
    ...
    mainObj.client.query(SQL1, Values1).then(() => {
        let SQL2 = `SELECT * FROM auth WHERE email=$1`
        let Values2 = [email]
        mainObj.client.query(SQL2, Values2).then((data) => {
            let SQL3 = `INSERT INTO person (first_name,last_name,phone,job_title,country,auth_id) VALUES ($1,$2,$3,$4,$5,$6);`;
            let Values3 = [first_name, last_name, phone, job_title, country, data.rows[0].id];
            mainObj.client.query(SQL3, Values3).then(() => {
                obj.sessionData = session_id;
                res.redirect('/');
            })
        })
...
obj.companySignUp = (req, res) => {
    let { company_name, email, phone, logo, country, company_url, account_type, session_id, password } = req.body;
    let SQL1 = `INSERT INTO auth (email,password,account_type,session_id) VALUES ($1,$2,$3,$4);`
    let Values1 = [email, password, account_type, session_id];
    mainObj.client.query(SQL1, Values1).then(() => {
        let SQL2 = `SELECT * FROM auth WHERE email=$1`
        ...
        mainObj.client.query(SQL2, Values2).then((data) => {
            ...
            let Values3 = [company_name, phone, logo, country, company_url, data.rows[0].id];
            mainObj.client.query(SQL3, Values3).then(() => {
                obj.sessionData = session_id;
                res.redirect('/');
            })
